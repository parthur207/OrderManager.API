...
using OrderManager.Application.RepositoryInterface;
...
namespace OrderManager.Infrastructure.Repository.Admin
{
    public class CheckTimeOccurenceOrderRepository : ICheckTimeOccurrenceOrderInterfaceRespository
    {
        ...
        public async Task<SimpleResponseModel> CheckTimeRepository(int orderNumber, ETypeOccurrenceEnum TypeOccurrence)
        {
            ...
            try
            {
                ...
                if (order.Occurrences.Any(x=>x.TypeOccurrence.Equals(TypeOccurrence)) && order.UpdatedAt)
                {
